...
public class TypeFactory
{
	...
	public static AttributeHandleSet newAttributeSet( int... handles )
	{
		try
		{
			AttributeHandleSet set = new HLA1516eAttributeHandleSet();
	...
	public static FederateHandleSet newFederateSet( int... handles )
	{
		try
		{
			FederateHandleSet set = new HLA1516eFederateHandleSet();
			for( int handle : handles )
			{
				set.add( new HLA1516eHandle(handle) );
			}
	...
	public static AttributeHandleValueMap newAttributeMap( Map<Integer,byte[]> values )
	{
		try
		{
			AttributeHandleValueMap map = new HLA1516eAttributeHandleValueMap();
			for( Integer handle : values.keySet() )
			{
				map.put( new HLA1516eHandle(handle), values.get(handle) );
			}
			return map;
		}
		catch( Exception e )
	...
	{
		return new HLA1516eAttributeHandleValueMap();
	}
	...
	public static ParameterHandleValueMap newParameterMap( Map<Integer,byte[]> values )
	{
		try
		{
			ParameterHandleValueMap map = new HLA1516eParameterHandleValueMap();
			for( Integer handle : values.keySet() )
			{
				map.put( new HLA1516eHandle(handle), values.get(handle) );
			}
	...
	{
		return new HLA1516eParameterHandleValueMap();
	}
