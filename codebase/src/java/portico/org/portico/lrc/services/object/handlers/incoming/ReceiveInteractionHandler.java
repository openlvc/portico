...
public class ReceiveInteractionHandler extends LRCMessageHandler
{
	...
	{		
		...
		if( logger.isDebugEnabled() )
		{
			...
			logger.debug( "@REMOTE Received INTERACTION [" + icMoniker(classHandle) +
			              "] with parameters " + pcMoniker(parameters.keySet()) +
			              ddmStatus + timeStatus );
		}
		...
		if( interest == null )
		{
			...
			logger.debug( "DISCARD interaction of class ["+icMoniker(classHandle)+"]: no subscription" );
		...
		if( notice.usesDDM() && !regionsOverlap(interest.getRegions(ourHandle),notice) )
		{
			...
			logger.debug( "DISCARD interaction of class ["+icMoniker(classHandle)+
	 ...
	 */
	private HashMap<Integer,byte[]> filter( ICMetadata subscribed, Map<Integer,byte[]> raw )
		throws Exception
	{
	 ...
	 */
	private boolean regionsOverlap( Set<RegionInstance> subscribed, SendInteraction notice )
		throws JRTIinternalError
	{
		...
		RegionInstance sentRegion = regions.getRegion( receivedToken );
		if( sentRegion == null )
		{
