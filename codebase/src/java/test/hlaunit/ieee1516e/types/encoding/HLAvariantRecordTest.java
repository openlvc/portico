...
import org.portico.impl.hla1516e.types.encoding.HLA1516eOctet;
import org.testng.Assert;
import org.testng.annotations.AfterClass;
import org.testng.annotations.BeforeClass;
import org.testng.annotations.Test;

@Test( sequential = true,groups = {"HLAvariantRecordTest", "types", "encoding"} )
public class HLAvariantRecordTest extends Abstract1516eTest
{
	...
	private void addTestFieldsToRecord( HLAvariantRecord record, boolean setValues )
	{
		...
		{
            record.setVariant(
                    this.encoderFactory.createHLAoctet(DISCRIMINANT_ONE),
                    this.encoderFactory.createHLAfloat32BE(VALUE_ONE)
            );
            record.setVariant(
                    this.encoderFactory.createHLAoctet(DISCRIMINANT_TWO),
                    this.encoderFactory.createHLAASCIIstring(VALUE_TWO)
            );
            record.setVariant(
                    this.encoderFactory.createHLAoctet(DISCRIMINANT_THREE),
    ...
    public void testHLAvariantRecordCreate()
    {
    	...
    	HLAvariantRecord defaultConstructor =
                this.encoderFactory.createHLAvariantRecord(
                        this.encoderFactory.createHLAoctet(DISCRIMINANT_ONE)
                );
    ...
    public void testHLAvariantRecordGetUnknownDiscriminant()
    {
    	HLAvariantRecord data =
                this.encoderFactory.createHLAvariantRecord(
                        this.encoderFactory.createHLAoctet((byte)4)
                );
    ...
    public void testHLAvariantRecordEncodeNoVariant()
    {
        ...
        data.setVariant(
                this.encoderFactory.createHLAoctet(DISCRIMINANT_FOUR),
                null
        );
