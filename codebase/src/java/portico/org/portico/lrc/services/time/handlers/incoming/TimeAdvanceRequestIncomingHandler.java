...
public class TimeAdvanceRequestIncomingHandler extends LRCMessageHandler
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	{		
		...
		// check to make sure we know about the source federate
		vetoIfSourceNotJoined( federate );
		...
		{
			// only tell the status of the advancing request if it IS NOT us, if it is
			// us, we will have already set this in the outgoing handler.
	...
	private void advanceFederate( int federateHandle, TimeStatus status )
	{
		...
		if( logger.isDebugEnabled() )
		{
			logger.debug( "ADVANCE (QUEUED) time-advance grant for federate ["+
			              moniker(federateHandle)+"] to time ["+status.getRequestedTime()+"]" );
		}
	}
