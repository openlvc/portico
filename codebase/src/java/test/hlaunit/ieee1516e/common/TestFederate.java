...
public class TestFederate
{
	...
	public AttributeHandleSet createAHS( int...handles )
	{
		try
		{
			...
			{
				set.add( new HLA1516eHandle(handle) );
			}
			return set;
		}
		catch( Exception e )
	...
	////////////////////////////////////////////////////////////
	/**
	 * Calls connect() on the RTI to establish the connection before we go ahead and join.
	 */
	...
	/**
	 * Checks to see if we're already connected to the RTI. If we're not, run
	 * {@link #quickConnect()}, otherwise, skip.
	 */
	public void quickConnectIfNot()
	{
	...
	public void quickDisconnect()
	{
		...
		{
			Assert.fail( "Exception in quickDisconnect in "+simpleName+" ("+e.getClass()+")", e );
		}
	}

	/**
	 * Checks to see if the federate has connected to the RTI. Returns true if it
	 * has, false otherwise.
	 */
	public boolean isConnected()
	{
		try
		{
			((Rti1516eAmbassador)this.rtiamb).getHelper().checkConnected();
			return true;
		}
		catch( Exception e )
		{
	 ...
	 * will be used.
	 * <p/>
	 * Note that you should have previously called {@link #quickConnect()}.
	 */
	...
	public void quickLocalDelete( int handle )
	{
		...
		fedamb.removeObjectInstance( TypeFactory.getObjectHandle(handle), "".getBytes(), null, null );
	}
