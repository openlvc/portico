...
public class AttributesAcquiredIncomingHandler extends LRCMessageHandler
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	{
		OwnershipAcquired notice = context.getRequest( OwnershipAcquired.class, this );
		int object = notice.getObjectHandle();
		Set<Integer> attributes = notice.getAttributeHandles();
		
		if( logger.isDebugEnabled() )
		{
			logger.debug( "ACQUIRE SUCCESS for object [%s] with attributes %s",
