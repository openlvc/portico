...
public class LRCMessageQueue //implements SaveRestoreTarget
{
	...
	protected LRCMessageQueue( LRCState lrcState )
	{
		...
		this.timeStatus = lrcState.getTimeStatus();
	}
	...
	public boolean offer( PorticoMessage message )
	{
		...
		{
			// notify the queue logger
			if( state.isQueueLogging() )
				logOffer( message );
	...
	public PorticoMessage poll()
	{
		...
		try
		{
			...
			if( this.roQueue.isEmpty() == false )
			{
				...
				//      -Can only deliver callbacks that are NOT "messages" (reflections, //
				//       interactions and delete calls are all "messages")                //
				...
				if( timeStatus.isConstrained() &&
					timeStatus.isAsynchronous() == false &&
					timeStatus.isAdvanceRequestOutstanding() == false )
				{
			...
			if( message != null )
			{
				...
				//   1) less than or equal to the local federate's current or requested time
				//                           -OR-
	...
	private void logOffer( PorticoMessage message )
	{
		if( state.theLRC.getLogger().isTraceEnabled() )
		{
			state.theLRC.getLogger().trace( "(offer) type=%s, src=%d, tgt=%s, time=%f",
			                                message.getType(),
			                                message.getSourceFederate(),
			                                message.getMultipleTargets(),
			                                message.getTimestamp() );
			state.theLRC.getLogger().trace( this );
		}
	}
	...
	private void logPoll()
	{
		if( state.theLRC.getLogger().isTraceEnabled() )
		{
			state.theLRC.getLogger().trace( "(poll) queue state after poll call:" );
			state.theLRC.getLogger().trace( this );
		}
	}
