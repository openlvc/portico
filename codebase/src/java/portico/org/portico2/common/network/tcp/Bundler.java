...
public class Bundler
{
	...
	////////////////////////////////////////////////////////////////////////////////////////
	/**
	 * Same as {@link #submit(Type, int, byte[])}, passing {@link PorticoConstants#NULL_HANDLE}
	 * for the request ID (meaning _no_ request id). Use for data messages only.
	 * 
	 * @param header The type of the message
	 * @param message The payload of the message
	 */
	public void submit( Type header, byte[] message )
	{
		submit( header, PorticoConstants.NULL_HANDLE, message );
	}
	 ...
	 * 
	 * @param header The type of message this is
	 * @param requestId The ID of the request to correlated requests and responses
	 * @param message the body of the message to send
	 */
	public void submit( Type header, int requestId, byte[] message )
	{
		lock.lock();
		...
		try
		{
			...
			if( logger.isTraceEnabled() )
				logQueuedMessage( header, requestId, message );
	 ...
	 */
	private final void logQueuedMessage( Type type, int requestId, byte[] payload )
	{
		if( logger.isTraceEnabled() == false )
			return;
		
		if( type == Type.CONTROL_REQ_ASYNC ||
			type == Type.CONTROL_REQ_SYNC ||
		...
		{
			logger.trace( "(outgoing) type=%s (id=%d), size=%d",
			              type,
			              requestId,
			              payload.length );
		}
	...
	private void flush()
	{
		...
		try
		{
			...
			outstream.writeByte( Type.BUNDLE.getByteValue() );
