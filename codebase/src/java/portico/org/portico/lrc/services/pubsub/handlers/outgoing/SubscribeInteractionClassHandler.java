...
public class SubscribeInteractionClassHandler extends LRCMessageHandler
{
	...
	public void process( MessageContext context ) throws Exception
	{
		...
		{
			String message = "ATTEMPT Subscribe to interaction class ["+icMoniker(classHandle)+"]";
			if( request.usesDdm() )
				message += " (region: "+regionToken+")";
			logger.debug( message );
		}
		
		// store the interest information -- regionToken will be NULL_HANDLE if DDM not used
		...
		{
			String message = "SUCCESS Subscribed to interaction class ["+icMoniker(classHandle)+"]";
			if( request.usesDdm() )
				message += " (region: "+regionToken+")";
			logger.info( message );
		}
