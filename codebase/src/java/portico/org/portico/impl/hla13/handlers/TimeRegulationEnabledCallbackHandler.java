...
public class TimeRegulationEnabledCallbackHandler extends HLA13CallbackHandler
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	public void process( MessageContext context ) throws Exception
	{
		if( logger.isTraceEnabled() )
			logger.trace( "CALLBACK timeRegulationEnabled(time="+lrcState.getCurrentTime()+")" );

		if( isStandard() )
		{
			DoubleTime currentTime = new DoubleTime( lrcState.getCurrentTime() );
			hla13().timeRegulationEnabled( currentTime );
		}
		else
		{
			byte[] currentTime = EncodingHelpers.encodeDouble( lrcState.getCurrentTime() );
			java1().timeRegulationEnabled( currentTime );
		}
